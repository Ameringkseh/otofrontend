          ...
          <div className="w-full max-w-md mx-auto animate-fadeIn">
            ...
            <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-700/60 rounded-3xl p-8 shadow-2xl shadow-black/50">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {/* Role selector (Register saja) */}
                {isRegister && (
                  <div className="space-y-1.5 animate-fadeIn">
                    <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider">Daftar Sebagai</label>
                    <div className="flex gap-3">
                      {['user', 'admin'].map((r) => (
                        <label key={r}
                          className={`flex-1 flex items-center gap-3 p-3.5 rounded-xl border cursor-pointer transition-all duration-200 ${formData.role === r
                          ...
                          <span className="text-slate-300">
                            {r === 'user' ? <User className="w-5 h-5" /> : <ShieldCheck className="w-5 h-5" />}
                          </span>
                          <div>
                            <p className="text-sm font-semibold capitalize">{r}</p>
                            <p className="text-xs opacity-70">{r === 'user' ? 'Anggota biasa' : 'Pengelola'}</p>
                          </div>
                          {formData.role === r && (
